...
const AgentAccessModal = ({ isOpen, onClose, agentId, agentName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/ai-agents/${agentId}/share`, {
        method: "POST",
        headers: {
                ...
                <div className="mb-6 p-4 bg-white/5 border border-white/20 rounded-xl">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${useMyDetails ? 'bg-[#70CBCF]' : 'bg-gray-600'
                        }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${useMyDetails ? 'translate-x-6' : 'translate-x-1'
                          }`}
                      />
                    </button>
                  </div>
              ...
              <form onSubmit={() => setIsConsentOpen(true)} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border border-white/30 bg-black/50 rounded-xl outfit-regular focus:outline-none focus:border-[#70CBCF] focus:ring-2 focus:ring-[#70CBCF]/20 text-base text-white placeholder-gray-500 transition-all duration-200 ${user && useMyDetails ? 'opacity-60 cursor-not-allowed' : ''
                      }`}
